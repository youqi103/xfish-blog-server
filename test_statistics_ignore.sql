-- ========================================
-- 简化版行为统计测试数据生成脚本（INSERT IGNORE版）
-- ========================================
...
WHERE 
    DATE_ADD(@start_date, INTERVAL FLOOR(RAND() * 30 * 24 * 3600) SECOND) <= @end_date
LIMIT 300;
...
-- 3. 生成评论记录数据
-- ========================================
INSERT IGNORE INTO comments (article_id, user_id, parent_id, content, status, ip_address, user_agent, created_at)
SELECT 
    FLOOR(1 + RAND() * 50),
    FLOOR(6 + RAND() * 13),
    CASE 
